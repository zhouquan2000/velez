...
# --- 2. 第三方库 ---
# (app.py 中未直接使用 ibapi，通过 contexts 间接调用)
...
from .events import on_exec_details, on_bar_update
from .shared import on_tws_error
...
async def main():
    ...
    sys_log("🚀基于Velex策略的量化交易程序,加载股票的配置参数", level="System")
    await asyncio.sleep(1.0)  # 确保 IB API 连接稳定
    start_funds = await show_account_detail()
    sys_log(
        f"💰[TWS账户] 资金查询。当前可用资金总额: ${start_funds:,.3f} USD",
        level="System",
    )
    ...
    contexts_placeholder.update(
        {
            symbol: TradingContext(
                symbol, ib, loop=loop, **STOCK_CONFIGS.get(symbol, {})
            )
            for symbol in SYMBOLS
        }
    )

    # 1. execDetailsEvent - 模块级函数绑定（一个绑定处理所有 symbol）
    ib.execDetailsEvent += on_exec_details

    # 2. commissionReportEvent - 为每个 ctx 实例绑定（需要逐个绑定）
    for symbol, ctx in contexts_placeholder.items():
        ib.commissionReportEvent += ctx.on_commission_report
        ctx.log(f"📡 佣金报告回调已绑定")

    # 3. errorEvent - 全局错误监听器
    ib.errorEvent += on_tws_error
    sys_log("📥 TWS 系统报错监听器函数已加载...")
    # ======================================================================
    for symbol, ctx in contexts_placeholder.items():
        sys_log(
            f"📋{symbol}|配置参数|"
            f"EB 门槛:{ctx.eb_range_mult}x | EB 量能:{ctx.eb_vol_mult}x | "
    ...
    for ctx in contexts:
        try:
            ...
            if qualified:
                ...
                ctx.log(
                    f"❌ 合约认证失败：请确认 {ctx.symbol} 是否有行情权限或拼写正确",
                    level="ERROR",
                )
    ...
    if init_tasks:
        ...
        for i, res in enumerate(results):
            ...
            if res is not True:
                ctx.suspend_today = True
                sys_log(
                    f"⚠️ [{ctx.symbol}] 历史数据初始化失败，已进入停摆模式。原因: {res}",
                    level="WARN",
                )
            else:
    ...
    sync_tasks = [
        ctx.sync_initial_state()
        for ctx in contexts_placeholder.values()
        if not ctx.suspend_today
    ]
    ...
    for symbol, ctx in contexts_placeholder.items():
        ...
        if ctx.suspend_today:
            ctx.log(
                f"🚫 {symbol} 今日暂停交易，不再与TWS服务器进行数据同步", level="ERROR"
            )
            continue
        ...
        try:
            ...
            ctx.log(
                f"📡 {symbol}与TWS服务器的实时链路已经建立，接收5秒bar数据 (RTH: Off)"
            )
        except Exception as e:
            ctx.log(
                f"❌ {symbol}与TWS服务器的数据链路异常，错误代码: {e}", level="ERROR"
            )
